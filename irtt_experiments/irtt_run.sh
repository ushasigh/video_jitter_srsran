...
experiment_name="irtt_5gsa_ul_dummy_022123"
...
for l in "${l_values[@]}"; do
  for i in "${i_values[@]}"; do
    for j in {1..2}; do
      ...
      irtt client -i "$i" -d 10s -l "$l" --fill=rand --sfill=rand -q 172.16.0.2 >> "$output_folder/irtt_${l}_${i}.txt"
    done
  done
done
